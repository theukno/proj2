      ...
      <CardContent className="p-4">
        <div className="flex gap-4">
          <div className="relative h-24 w-24 rounded-md overflow-hidden">
            <Image src={item.image || "/placeholder.svg?height=200&width=200"} alt={item.name} fill className="object-cover" onError={(e) => e.currentTarget.src = "/placeholder.svg?height=200&width=200"} />
          </div>
...
function LoginDialog({ isCheckoutOpen, setIsCheckoutOpen, setIsLoggedIn }) {
  return (
    <Dialog open={isCheckoutOpen} onOpenChange={setIsCheckoutOpen}>
      <DialogContent>
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-2">
          <Button variant="outline" className="sm:flex-1" onClick={() => { setIsCheckoutOpen(false); router.push("/account"); }}>Create Account</Button>
          <Button className="sm:flex-1" onClick={() => { localStorage.setItem("loggedInUser", JSON.stringify({ email: "user@example.com" })); setIsLoggedIn(true); setIsCheckoutOpen(false); router.push("/checkout"); }}>Sign In & Checkout</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
